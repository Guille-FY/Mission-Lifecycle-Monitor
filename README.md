...
The system uses a modern observability stack for log management:

*   **Log Backend**: **Grafana Loki**. The architecture includes a Loki instance (port 3100) for high-performance log aggregation.
*   **Live Visualization**: The Mission Control dashboard acts as a real-time **"Live Tail"** client. For this demonstration, it polls the Flight Computer's immediate state buffer to achieve 60fps animations without the latency overhead of querying the database directly.
*   **Format**: Structured JSON logs are generated by the backend and are compatible with OpenTelemetry log data models.

1.  **Generation**: Flight Computer produces structured events.
2.  **Aggregation**: Events are exposed for both real-time UI consumption and system retention.
...
Below is the complete reference of all events generated by the simulation:
    ...
    ```javascript
    { timestamp: "2026-01-28...", message: "MAX Q reached", type: "INFO" }
    ```
2.  **Exposition**: The `GET /telemetry` endpoint exposes the entire rocket state, including this events array.
3.  **Ingestion**: The Mission Control frontend polls this endpoint every 1000ms.
4.  **rendering**: The Terminal component diffs the new events and renders them with the appropriate color coding (Green=Success, Yellow=Warning, Red=Error).
...
*   **Observability**: OpenTelemetry (Node.js SDK), Docker, Prometheus, Jaeger, **Grafana Loki**.
